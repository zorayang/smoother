...
sq_s <- data_s[,1]
p_pred_s <- poly_approx(data_s[,1], data_s[,2], sq, n = 5)
...
s_y <- test_function(sq, type = "Sine")
...
sq_w <- data_w[,1]
p_pred_w <- poly_approx(data_w[,1], data_w[,2], sq, n = 10)
...
```{r}
#Sine
sp_pred_s <- natural_spline(data_s[,1], data_s[,2], sq, k = 5)
plot(data_s[,1], data_s[,2], xlab = "x", ylab = "y", pch = 20, cex = .5, 
...
points(sq_w, sp_pred_s, pch = 20, col = "hotpink")
lines(sq_w, s_y, col = "slateblue")
```